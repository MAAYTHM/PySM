...
class Gather_tweets(object):
    ...
    def gather_tweets(self, client, max_results, limit,
                      next_token, tweets_list, tweets_dict,
                      temp_goal, Total_count):
        global exit_event, delimiter, Total_requests

        """
        This Function do :-
            * Build query to search on twitter API.
            * Gather tweets from twitter API.
            * If "fast-mode" is OFF, then Call "SupportPurify" Class also here which take time.
        """

        # Local function variables
        query = f'("{self.keyword}" OR \
"{self.keyword.upper()}" OR \
"{self.keyword.capitalize()}") \
lang:en has:mentions -is:retweet'
        function_count = 0
        break_flag = False
        ...
        for i in range(limit):
            ...
            if Total_requests == Twitter_rate_limit_rq:
                error_display(desc="\tTwitter Rate limit reached, \
Retry After 15 minutes - \
\n[ info ] Processing tweets which we gathered till",
                              exit=False)
                break_flag = True
            ...
            try:
                ...
                error_display(desc=f"Invalid Bearer Token \
            ...
            for tweet in response.data:
                ...
                if tweet not in tweets_list:
                    ...
                    if not self.allow_uniq:
                        ...
                        # increasing function count without checking length ,
                        #  if "-u" is not set
                        function_count += 1

                        # Showing Progress (without "-u") :-
                        if self.show_progress:
                            custom_print(f"\t\tTweets gathered - \
{Total_count + function_count} tweets", end='\r')

                        # If goal achieved in mid of checking tweets,
                        # then exit
                        if temp_goal == function_count:
                            break_flag = True
                            break
            ...
            if self.allow_uniq:
                ...
                supporter = SupportPurify()
                pure_tweets = supporter.clear_tweet(tweet=delimiter_tweets)
                pure_tweets = pure_tweets.split(local_delimiter)
                ...
                for index, pure_tweet in enumerate(pure_tweets):
                    ...
                    if pure_tweet not in tweets_dict:
                        tweets_dict[pure_tweet] = [tweets_list[index],
                                                   "none"]
                        function_count += 1

                    # Showing Progress (with "-u") :-
                    if self.show_progress:
                        custom_print(f"\t\tTweets gathered - {Total_count + function_count} \
tweets", end='\r')
        ...
        # Returning
        # How many uniq tweets we gathered in this function
...
class Purifier(Thread):
    ...
    def run(self):
        ...
        This Function do :-
            * Take one tweet at a time and Purify it using "SupportPurify" Class.
        """

        while True:

            # If there is anything in the list remaning then ,
            #  try condition will work
            try:
            ...
            # Purifying Tweet
            pure_tweet = SupportPurify().clear_tweet(tweet=tweet)
...
class SupportPurify():
    ...
    def clear_tweet(self, tweet):
        ...
        """
        This Function do :-
            * Purify Tweets with several steps.
        """
        ...
        new_text = re.sub(r"http\S+",
                          " ", new_text)
        new_text = re.sub(r"www\S+",
                          " ", new_text)

        # Step - Replacing HTML Entities
        # like - '&amp;', '&quot;' etc..
        new_text = str(replace_entities(new_text))

        # Step - Removing Punctuations
        new_text = pattern_punctuation.sub(' ',
                                           new_text)

        # Step - Removing Stopwords
        new_text = pattern_stopwords.sub(' ',
                                         new_text)
        ...
        # Step - Lemmatizing each word
        # with POST(Parts Of Speech Tagging)
        ...
        for word, pos in pos_data:
            ...
            if (word.lower() == keyword
                    or (unique and word == self.delimiter)):
                " "*space_var + word
            try:
                pos = pos_dict[pos[0]]
    ...
    def spell_corrector(self, tweet):
        ...
        """
        This Function do :-
            * Correct wrong written word in tweet.
    ...
    def emojis_to_text(self, text):
        ...
        """
        This Function do :-
            * Convert Emojis to their meaning text.
        """
...
class Analyser(Thread):
    ...
    def run(self):
        ...
        (Threads target function)
        This Function do :-
            * take one tweet at a time from list and send to "which_sentiment" function.
        """

        # Running ♾️loop, which will break
        # when there's nothing in tweets list
        while True:
            try:
    ...
    def which_sentiment(self, tweet):
        ...
        """
        This Function do :-
            * Main function which extract sentiment from purified tweets and return.
        """
        ...
        if max_value == 'pos' or (max_value == 'compound' and
                                  value_dict[max_value] >= 0.5):
            return '+ve'

        elif max_value == 'neg' or (max_value == 'compound' and
                                    value_dict[max_value] <= -0.5):
            return '+ve'
...
def custom_print(*args, **kwargs):
    ...
    """
    This Function do :-
        * same work that "print" do in python -> printing lines.
        * not print other than errors, if "--silent" is used.
    """
    ...
    if not silent and not exit_event.is_set():
        print(print_data, end=end)

    # If any error occur then print it,
    #  even with "--silent" flag
    elif error:
        print(print_data, end=end)
...
def clear_screen():
    ...
    """
    This Function do :-
        * Clear terminal screen for better output look, on the basis of SYSTEM OS.
    """
...
def main():
    global keyword, max_results, output_filename,\
        max_threads, show_progress, silent, unique, all_yes, token

    """
    This Function do :-
        * As it name says, It is the 'main' function which start every step and end with "show_piechart" function.
        * Display Info Banner.
        * Calculate 'number of threads'.(If user not used "-t").
    """
    ...
    if not silent:
        ...
        custom_print(f"Max Threads Allowed (-t) - {no_of_threads} \
(based on allowed Max results, Max threads and some Calculation)")
        custom_print(f"Fast Mode (-f) - {not unique}")
        custom_print(f"All Answer 'Yes' (-y) - {all_yes}")
        custom_print("#"*10, " INFO BANNER ", "#"*10)
        custom_print()

    # STEP 1 -> "Gathering Uniq Tweets"
    Step_1(total_results=max_results,
           Original_tweets=Original_tweets,
           All_uniq_tweets=All_uniq_tweets,
           Total_count=Total_count,
           Total_time = Total_time)

    # STEP 2 -> "Threading For Puryfying Tweets"
    if not unique:
        Step_2(Original_tweets=Original_tweets,
               All_uniq_tweets=All_uniq_tweets, 
               no_of_threads=no_of_threads,
               Total_time = Total_time)

    # STEP 3 -> "Threading For Sentiment Analyzing"
    Step_3(All_uniq_tweets=All_uniq_tweets,
           no_of_threads=no_of_threads,
           Total_time = Total_time)

    # showing How much request send to twitter,
    #  for collecting tweets
    custom_print(f"Total_requests -> \
...
# STEP 1 -> "Gathering Uniq Tweets"
def Step_1(
            total_results, Original_tweets,
            All_uniq_tweets, Total_count, Total_time):
    ...
    """
    This Function do :-
        * Gather Tweets from twitter, using "Gather_tweets" Class.
        * If "fast-pace" mode is OFF, then It will Purify threads here only.
        * Handle Twitter Rate limit.
    """

    if unique:
        custom_print(f"Step 1 - Gathering & Purifying Tweets \
(Started)")
    else:
        custom_print(f"Step 1 - Gathering Tweets \
(Started)")
    ...
    # Local function variables
    # 450 * 100 -> 45,000 tweets can gather in 15 min.
    Twitter_search_limit = Twitter_rate_limit_rq * 100

    # 15 mins in seconds -> 900 sec
    Twitter_wait_time = Twitter_rate_limit_min * 60
    ...
    # FOr if "-u" set, becoz of the "SupportPurify" CLass
    if unique:
        Total_time_taking = total_results // 3

    # Displaying Info Banners
    custom_print(f"\t[!] According to Twitter API docs, \
Rate Limit (Over Search Tweets) is \
{Twitter_rate_limit_rq} requests / {Twitter_rate_limit_min} min.")
    custom_print("\t[!] means, \
This Tool can only gather around 44.5k tweets per 15 minutes.")
    custom_print(f"\tMax time to gather ({total_results} tweets) -> \
{Total_time_taking} sec")
    custom_print()
    if show_progress:
        custom_print("\tProgress INFO :-")
    ...
    while Remaining_result != 0:
        ...
        # "temp_limit" is simillar to "remaining_result",
        # only diff is -> "temp_limit" is a local variable for this loop,
        # where "remaining_result" is a global variable
        temp_limit = Remaining_result % Twitter_search_limit

        # for handling if "remaining_result" == "45000"
        if Remaining_result >= Twitter_search_limit:
            temp_limit = Twitter_search_limit
        ...
        # "extra_results" -> 2 digit no_of_requests out of big "no_of_requests".
        # like 11 out of 1011, 10 outof 1010, 1 outof 101.
        # Not 2 digit request like 89, 12, 45 etc ...
        max_results = 100
        extra_results = temp_limit % 100
        if temp_limit < 10:
            extra_results = 10

        # changing "extra_results" to 100, if "-u" flag is set.
        # else "-u" cost very high in total number of request
        if unique and extra_results:
            extra_results = 100

        # calculating pages needed to surf (twitter API Tweets pages with 'next_token') ,
        #  via "temp_limit" variable.
        # Only for those no. of request which is divisible by 100 and remainder is "0".
        no_of_pages = temp_limit // 100

        # Checking "Total_requests" > "Twitter_rate_limit_rq"
        # so that in bw function loop, no error occur, part - 1
        if Total_requests > Twitter_rate_limit_rq:
            # format printing
            if show_progress:
                custom_print()
...
which we gathered till", exit=False)
            break

        # calling main function & Updating "temp_goal_achieved"
        #  according to results achieved
        if no_of_pages:
            temp_1, Next_token = Gatherer.gather_tweets(client=Client,
                                                        max_results=max_results, limit=no_of_pages,
                                                        next_token=Next_token, tweets_list=Original_tweets,
                                                        tweets_dict=All_uniq_tweets,
                                                        temp_goal=temp_goal, Total_count=Total_count)
            temp_goal_achieved += temp_1

        if extra_results:
            temp_1, Next_token = Gatherer.gather_tweets(client=Client,
                                                        max_results=extra_results, limit=1, next_token=Next_token,
                                                        tweets_list=Original_tweets, tweets_dict=All_uniq_tweets,
                                                        temp_goal=temp_goal, Total_count=Total_count)
            temp_goal_achieved += temp_1
...
# STEP 2 -> "Threading for Puryfying Tweets"
def Step_2(Original_tweets, All_uniq_tweets, 
            no_of_threads, Total_time):
    global exit_event

    """
    (If "fast-paced" mode ON, then This step will run)
    This Function do :-
        * Purify tweets with the help of threads and Purifier Class.
    """

    # a thread Start thread
    def start_threads(
                no_of_threads, Original_tweets,
                All_uniq_tweets, threads):
        global break_event

        """
        This Function do :-
            * Start threads and assign them work.
        """

        # variables for CLass parameters
        temp_original_tweets = []
        temp_original_tweets.extend(Original_tweets)

        for _ in range(no_of_threads):
            if not break_event.is_set():
                thread = Purifier(tweets_list=temp_original_tweets,
                                  tweets_dict=All_uniq_tweets)
    ...
    custom_print(f"\tMax time to 'PURIFY' (6 tweets/sec) = \
{round((len(Original_tweets) // 6), 2)} sec. \
(Depend On number of 'threads and tweets')")
    custom_print()

    # Starting all threads with a thread
    starting_thread = Thread(target=start_threads,
                             kwargs={"no_of_threads": no_of_threads,
                                     "Original_tweets": Original_tweets,
                                     "All_uniq_tweets": All_uniq_tweets, "threads": threads})
...
def Step_3(All_uniq_tweets, 
            no_of_threads, Total_time):
    global exit_event

    """
    This Function do :-
        * Sentiment Extraction From Purified Tweets with the help of 'Analyser' Class.
        * Use threads to speed up work.
    """
    ...
    def start_threads(
        ...
        for _ in range(no_of_threads):
            if not break_event.is_set():
                thread = Analyser(tweets_dict=All_uniq_tweets,
                                  tweets=tweets, analyzer=analyzer)
                threads.append(thread)
                thread.start()
            else:
                break
    ...
    custom_print(f"\tMax time to 'PURIFY' ((\
{len(All_uniq_tweets)} tweets / 100) x 2) = {total_time} sec")
    custom_print()
    ...
    if exit_event.is_set():
        sys.exit(f"\t(Step 3) Time taken :- {end_time} sec")
...
def calculate_results(tweets_dict, Total_time):
    global exit_event, keyword, output_filename

    """
    This Function do :-
        * Brand Reputation calculated here.
        * Pass the measurement to "show_piechart" function.
    """
    ...
    for key, values in tweets_dict.items():
        ...
        original_tweets.append(values[0])
    ...
    if output_filename:
        response_name_dict = {"+ve": "Positive Response",
                              "-ve": "Negative Response", "n": "Neutral Response"}

        tweets_df = pd.DataFrame(tweets,
                                 index=[response_name_dict[response]
                                        for response in responses],
                                 columns=['Original Tweets', 'Purified Tweets'])
        ...
        try:
            ...
            error_display(desc=f"Error \
                while saving in file ({output_filename}), ",
    ...
    if negative_count:
        ...
        while responses.count('-ve'):
            ...
            responses.remove('-ve')
            custom_print(f"{negative_count - responses.count('-ve')}) ",
                         tweet)
    ...
    if exit_event.is_set():
        sys.exit()
    custom_print()
    custom_print(f"Postive Responses - \
        {round((positive_count/len(tweets_dict))*100, 2)}% / 100%")
    custom_print(f"Negative Responses - \
        {round((negative_count/len(tweets_dict))*100, 2)}% / 100%")
    custom_print(f"Neutral Responses - \
        {round((neutral_count/len(tweets_dict))*100, 2)}% / 100%")
    custom_print()

    custom_print(f"\tAccording to Total({len(tweets_dict)}) Tweets Scanned :-")
    if (positive_count+neutral_count) > negative_count:
        custom_print(f"\t-> '{keyword}' has Nice Reputation. \
({((positive_count+neutral_count)/len(tweets_dict))*100}% / 100%) ")

    elif negative_count > positive_count:
        custom_print(f"\tSorry, But '{keyword}' has not such good Reputation. \
({(negative_count/len(tweets_dict))*100}% / 100%) ")
    ...
    # Printing "Total-Time"
    custom_print(f"[+] Total-Time taken :- {Total_time} sec\n")
...
def show_piechart(list_of_items):
    ...
    """
    This Function do :-
        * Display Piechart for better understanding.
    """
    ...
    # Only appending those 'key&value' pair,
    # which have some response value/tweet other than 0
    if list_of_items[0]:
        custom_dict["+ve opinion"] = [list_of_items[0], '#99ff99']
    if list_of_items[1]:
        custom_dict["-ve opinion"] = [list_of_items[1], '#ff4040']
    if list_of_items[2]:
        custom_dict["neutral opinion"] = [list_of_items[2], '#ffcc99']
    ...
    ax1.pie(myvalue, labels=mylabels, colors=mycolors,
...
def signal_handler(signum, frame):
    ...
    """
    This Function do :-
        * Set a signal If User pressed "Ctrl+C".
    """

    remove_progress_mark()
    error_display(desc="User Interrupted!!, 'Ctrl+C' Pressed \
| Closing all threads ...", exit=False)
    exit_event.set()
...
def lock():
    ...
    """
    This Function do :-
        * Start a while ♾️ loop ,till all other working threads over.
        * Display Number of threads / Progress bar for threads.
    """
    ...
    while (active_count() != 1):
        ...
        # Progress bar for threads
        if show_progress:
            custom_print(f"\tThere are {active_count() - 1} \
threads remaining...", end="\r")

        # printing line, if progress bar is not active
        else:
            custom_print(f"\t[?] Waiting for finish of the Step.",
                         end="\r")
...
# Helps to remove Progress marks,
# like -> "Tweets recvd 100/100",
# after progress bar complete
def remove_progress_mark():
    custom_print(" " * 100, end="\r")


# Showing errors with format
def error_display(desc, error="",
                  exit=True):
    ...
    error = str(error)
    if not error.isspace() and error:
        custom_print(f"Full error desc -> {error}\n")
...
Starting Code does :-
    ...
    * Checking cmdline argument's value is Valid.
    * Install necessary NLTK libraries. 
    * Create Global Variables for above functions.
    * Start "main()" function
...
if __name__ == '__main__':
    ...
    # Help menu
    description = "A Social media Scrapper tool for specifically Twitter \
to measure reponses of a particular keyword"
    epilog = ("""
...
Note:
    * By default, The tool run on Normal pace mode.""" +
    " So High chance that you will recv your desired max_result tweets." +
    """
...
Made by (😎):
    * MAAY (https://github.com/MAAYTHM)
    """)
    parser = AP(description=description, epilog=epilog,
                formatter_class=RawDescriptionHelpFormatter)

    # Setting Up cmdline arguments
    parser.add_argument('-k', '--keyword', metavar='KEYWORD',
                        type=str, help='Keyword to search.')

    parser.add_argument('-m', '--maxresult', metavar='MAX RESULTS',
                        type=int, help='How many tweets, want to scan? (default - 10) ',
                        default=10)

    parser.add_argument('-t', '--threads', metavar='THREADS',
                        type=int, help='Maximum Threads to allow, (default - auto, \
"but less_than 100")')

    parser.add_argument('-p', '--progress', metavar='OPTIONAL',
                        help='Show progress of tweets fetching. (default - "FALSE")',
                        default=False, nargs="?", const=True)

    parser.add_argument('-f', '--fast', metavar='OPTIONAL',
                        help='Need Tweets Fast? \
(It takes less time than normal but not sure \
to gather demanded max_results) (default - "FALSE")',
                        default=False, nargs="?", const=True)

    parser.add_argument('-o', '--output', metavar='CSV FILE PATH',
                        type=str, help='File name to save results (.csv) \
(default - "scrapper_result.csv")',
                        default="scrapper_result.csv")

    parser.add_argument('-y', '--yes', metavar='OPTIONAL',
                        help='If the program prompt for "yes"/"no", Then It will set "Yes" \
to each question (asked from the program) (default - FALSE)',
                        default=False, nargs="?", const=True)

    parser.add_argument('--silent', metavar="OPTIONAL",
                        help='Not print anything, ONLY errors will display \
and saving will be done. (default - FALSE)',
                        default=False, nargs="?", const=True)

    parser.add_argument('--install', metavar='REQUIRED ONCE',
                        help='Before running it first time for scraping, \
Use this flag to download all necessary nltk libraries',
                        default=False, nargs="?", const=True)
    ...
    # If "fast" mode ON, then off slow/uniq mode
    if arguments.fast:
        unique = False
    ...
    if install_lib:
        ...
        try:
            ...
            # Installing modules
            modules = ["demoji==1.1.0", "matplotlib==3.5.1",
                       "nltk==3.7", "pandas==1.4.1",
                       "tweepy==4.8.0", "w3lib==1.22.0"]
            ...
            error_display("Error Occured \
while Installing necessary modules.",
                          error=error)
        ...
        try:
            ...
            error_display("One or more necessary Modules are absent, \
run 'python socialmedia_scraper.py --install' once", error=error)

    # Setting Up "exit_event" & break_event for program :-
    # for Ctrl+c
    exit_event = Event()

    # for breaking starting_threads from for loop
    break_event = Event()
    ...
    Twitter_rate_limit_rq = 450
    # 450 requests per 15 min.
    Twitter_rate_limit_min = 15

    # Limiting "max_results"
    if max_results >= 45000:
        error_display("Too much Value, \
Maximum value can be '45000'.",
                      exit=False)
        ask_continue = input("Continue with 45000? (Y/n) ")
        if ask_continue.lower() != "y":
            # If user's input is not 'yes' then exit
            sys.exit("\nExiting...")
    ...
    try:
        ...
        # for windows
        if os.name == 'nt':
            token = str(json.load(open("resources\\config.json",
                                       "r"))['Bearer Token'])
        ...
        if not token and not token.isspace():
            raise Exception
...
\n[ tip-3 ] Checkout Installaton Page of this tool - \
'https://github.com/MAAYTHM/PySM'. ", error=error)
        ...
        if keyword is not None:
            ...
            if output_filename:
                ...
                if os.path.exists(output_filename):
                    ...
                    if all_yes:
                        custom_print(f"File ('{output_filename}') already exists. \
Delete it and then Proceed.\nDelete - [Y/n] -> Y")
                        ask_delete = "y"

                    else:
                        ask_delete = input(f"File ('{output_filename}') already exists. \
Delete it and then Proceed.\nDelete - [Y/n] -> ").lower()

                    custom_print()
                    if ask_delete == 'y' or not ask_delete:
                        os.remove(output_filename)
                        custom_print(f"File ({output_filename}) deleted successfully, \
Continuing Work ...")

                    else:
                        error_display('File not deleted, & Exits ...')

                    # Few seconds sleep
                    time.sleep(1)
                    clear_screen()

            # Variables for "SupportPurify" Class :-
            stopwords_set = set(stopwords.words("english"))
            pattern_stopwords = re.compile(r'\b(' +
                                           r'|'.join(stopwords_set) + 
                                           r')\b\s*')

            word_lemmatizer = WordNetLemmatizer()
            pos_dict = {'A': wordnet.ADJ, 'S': wordnet.ADJ_SAT,
                        'R': wordnet.ADV, 'N': wordnet.NOUN, 'V': wordnet.VERB}
            ...
            try:
                # for windows
                if os.name == 'nt':
                    words_dict = list(json.load(open("resources\
\\words_dict.json")).keys())
...
\n[ tip-2 ] Checkout Installaton Page of this tool - \
'https://github.com/MAAYTHM/PySM'. ", error=error)
            ...
            error_display(desc="the following arguments are required: \
-k/--keyword")

    except PermissionError as error:
        error_display(desc="Improper Permission OR Rerun this Tool.",
                      error=error)

    except RuntimeError or KeyboardInterrupt as error:
        error_display(desc="User Interrupted!!, 'Ctrl+C' Pressed \
| Closing all threads ...", error=error)
